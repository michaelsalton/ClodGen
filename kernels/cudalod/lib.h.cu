// Vendored from CudaLOD: modules/simlod/sampling_cuda_nonprogressive/lib.h.cu
...
//
// CudaLOD's device library: vec3/Box3, the non-atomic bump Allocator, Timer, and
// the debug-draw Points/Lines/Voxels structs.
//
// NOTE its Allocator has no capacity and no bounds check. The port keeps it as-is so
// the ported kernels stay byte-identical and verifiable against bench/reference/, and
// bounds the damage from the HOST instead, by sizing the slab from the worst
// strategy's measured watermark. See CudalodPipeline.
