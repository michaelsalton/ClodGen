// Vendored from CudaLOD: modules/simlod/sampling_cuda_nonprogressive/split_countsort_blockwise.h.cu
...
//
// Phase 1. A dense 256^3 main counting grid (depth 8) plus sparse 16^3 subgrids on
// overfull cells, giving effective depth 12.
//
// Upstream calls split_node ONCE, so a pathological cloud gets oversized leaves rather
// than a deeper tree. That is deliberately NOT fixed here -- it is the published
// algorithm and this is a reproduction. maxPointsPerNode is surfaced in the stats panel
// so you notice when it happens.
